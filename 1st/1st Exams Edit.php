...
$studentView = $studentController->viewAll1stStudentExams($center);
$studentEntry = $studentController->viewAll1stStudentExams($center);

$student = new StudentExams;
$student->num = $_GET['examNum'];
$student->center = $center;

if(isset($_POST["saveEdit"])){
    foreach($studentEntry as $stEntry){
        if(!empty($_POST[$stEntry['id'].'e'])){

            $student->name  = $stEntry['name'];
            $student->id    = $stEntry['id'];
            $student->class = '1st sec';
            $student->grade = trim($_POST[$stEntry['id'].'e']);

            if(!$studentController->newStudentExamGrade($student)){
                break ;  
            }
        }
    }
...
<head>
    ...
    <title>1st sec Edit '<?= $student->num ?>' Exam</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- DataTales Example -->
                    <form method="POST" action="1st Exams Edit.php?examNum=<?= $student->num ?>&edit=1&place=<?=$center?>">
                        <div class="card shadow mb-4">
                            <div class="card-header py-3">
                                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                                    <h6 class="m-0 font-weight-bold text-primary">Edit '<?= $student->num ?>' Exam...</h6>
                                    <button type="submit" class="btn btn-success shadow-sm" name="saveEdit" id="saveEdit">
                                        <i class="fas fa-check-circle"></i> Save
                                    </button>
                                    <a href="1st Exams.php?place=<?=$center?>" class="d-none d-sm-inline-block btn btn-google shadow-sm">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <?php
                                                    ...
                                                    $student->class = '1st sec';
                                                    $studentGrade = $studentController->viewStudentAllExams($student);
                                                ?>
                                                <td><input type="text" class="form-control form-control-user"
                                                        name="<?=$stView['id'].'e'?>" id="<?=$stView['id'].'e'?>" value="<?= trim($studentGrade["$student->num"])?>" 
